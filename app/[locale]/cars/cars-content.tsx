...
import {
  Search, SlidersHorizontal, X, Car, ChevronDown, Loader2,
  Users, ArrowUpDown, Fuel, Gauge, CalendarIcon, Plane, Building, ArrowRightLeft,
} from 'lucide-react'
...
const emptyFilters: Filters = {
  city: '', category: '', transmission: '', fuel_type: '',
  price_min: '', price_max: '', seats_min: '', sort: 'newest',
  pickup_type: '', return_same: true, return_city: '', pickup_date: '', return_date: '',
}
...
export function CarsContent({ initialCars, initialTotalPages, initialFilters }: CarsContentProps) {
  ...
  const [searchCity, setSearchCity] = useState(initialFilters.city)
  ...
  const fetchCars = useCallback(
    async (pageNum: number, append = false, overrideCity?: string) => {
      ...
      try {
        ...
        if (append) setCars((prev) => [...prev, ...(data.cars || [])])
  ...
  const filterDepsRef = useRef({
    category: initialFilters.category, transmission: initialFilters.transmission,
    fuel_type: initialFilters.fuel_type, price_min: initialFilters.price_min,
    price_max: initialFilters.price_max, seats_min: initialFilters.seats_min,
    sort: initialFilters.sort, pickup_type: initialFilters.pickup_type,
  })

  useEffect(() => {
    const prev = filterDepsRef.current
    const changed =
      prev.category !== filters.category || prev.transmission !== filters.transmission ||
      prev.fuel_type !== filters.fuel_type || prev.price_min !== filters.price_min ||
      prev.price_max !== filters.price_max || prev.seats_min !== filters.seats_min ||
      prev.sort !== filters.sort || prev.pickup_type !== filters.pickup_type
    if (changed) {
      filterDepsRef.current = {
        category: filters.category, transmission: filters.transmission,
        fuel_type: filters.fuel_type, price_min: filters.price_min,
        price_max: filters.price_max, seats_min: filters.seats_min,
        sort: filters.sort, pickup_type: filters.pickup_type,
      }
      setPage(1)
      fetchCars(1)
    }
  ...
  const clearFilters = () => { setFilters(emptyFilters); setSearchCity('') }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-8 md:pt-32 md:pb-16 lg:pt-36 lg:pb-20 animate-fade-in-up">
      <div className="text-center max-w-3xl mx-auto mb-8 md:mb-12">
        <h1 className="text-3xl md:text-4xl lg:text-5xl font-black text-slate-900 tracking-tight mb-3 md:mb-4">{t('cars.title')}</h1>
        <p className="text-base md:text-lg text-slate-500 font-medium px-4">{t('cars.subtitle')}</p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
          <Popover>
            ...
            <PopoverContent className="w-auto p-0" align="start">
              <Calendar mode="single" selected={filters.pickup_date ? parseISO(filters.pickup_date) : undefined} onSelect={(date) => updateFilter('pickup_date', date ? format(date, 'yyyy-MM-dd') : '')} disabled={(date) => date < new Date()} initialFocus />
            </PopoverContent>
          </Popover>
          ...
          <Popover>
            ...
            <PopoverContent className="w-auto p-0" align="start">
              <Calendar mode="single" selected={filters.return_date ? parseISO(filters.return_date) : undefined} onSelect={(date) => updateFilter('return_date', date ? format(date, 'yyyy-MM-dd') : '')} disabled={(date) => { const min = filters.pickup_date ? parseISO(filters.pickup_date) : new Date(); return date <= min }} initialFocus />
            </PopoverContent>
          </Popover>

          <Popover>
            <PopoverTrigger className={cn('flex h-12 md:h-14 w-full items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-4 text-sm font-semibold transition-colors hover:bg-slate-100', filters.category ? 'text-slate-700' : 'text-slate-500')}>
              <span className="flex items-center gap-2 truncate">
                <Car className="h-4 w-4 text-slate-400 shrink-0" />
                {filters.category ? (isAr ? CAR_CATEGORIES[filters.category as keyof typeof CAR_CATEGORIES]?.ar : CAR_CATEGORIES[filters.category as keyof typeof CAR_CATEGORIES]?.en) : t('cars.filter_category')}
              </span>
              <ChevronDown className="h-4 w-4 text-slate-400" />
            </PopoverTrigger>
            <PopoverContent className="w-64 p-2" align="start">
              <div className="grid gap-1">
                <Button variant={!filters.category ? 'secondary' : 'ghost'} className="h-10 justify-start rounded-xl" onClick={() => updateFilter('category', '')}>{t('cars.all_categories')}</Button>
                {Object.entries(CAR_CATEGORIES).map(([key, val]) => (
                  <Button key={key} variant={filters.category === key ? 'secondary' : 'ghost'} className="h-10 justify-start rounded-xl" onClick={() => updateFilter('category', key)}>{isAr ? val.ar : val.en}</Button>
                ))}
              </div>
            </PopoverContent>
          </Popover>
          ...
          <div className="flex items-center gap-3">
            <Popover>
              <PopoverTrigger className="flex h-10 w-40 items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-4 text-xs font-semibold text-slate-700 shadow-none transition-colors hover:bg-slate-100 md:w-48 md:text-sm">
                <span className="flex items-center gap-2 truncate"><ArrowUpDown className="h-4 w-4 text-slate-400" />{filters.sort === 'price_asc' ? t('cars.sort_price_asc') : filters.sort === 'price_desc' ? t('cars.sort_price_desc') : t('cars.sort_newest')}</span>
                <ChevronDown className="h-4 w-4 text-slate-400" />
              </PopoverTrigger>
            ...
            <Button variant={showFilters ? 'secondary' : 'outline'} onClick={() => setShowFilters(!showFilters)} className={cn('h-10 rounded-xl px-4 font-bold text-xs md:text-sm', showFilters && 'bg-accent text-white hover:bg-accent/90')}>
      ...
      {showFilters && (
        <div className="rounded-[1.5rem] md:rounded-[2rem] border border-slate-100 bg-white p-5 md:p-8 mb-8 md:mb-12 shadow-xl shadow-slate-200/40 animate-fade-in-up" style={{ animationDelay: '100ms' }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            ...
            <div className="space-y-1.5 md:space-y-2">
              <Label className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-500">{t('cars.filter_seats_min')}</Label>
              <Input type="number" min="1" value={filters.seats_min} onChange={(e) => updateFilter('seats_min', e.target.value)} placeholder="1" className="h-11 rounded-xl border-slate-200 bg-slate-50 text-sm font-medium text-slate-700 shadow-none hover:bg-slate-100" />
            </div>
            <div className="space-y-1.5 md:space-y-2">
              <Label className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-500">{t('cars.filter_price')} ({t('common.from')})</Label>
            ...
            <div className="space-y-1.5 md:space-y-2">
              <Label className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-500">{t('cars.filter_price')} ({t('common.to')})</Label>
        ...
        <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          {activeFilterLabels.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {activeFilterLabels.map((label) => (<Badge key={label} variant="outline" className="rounded-full border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-600 shadow-sm">{label}</Badge>))}
            </div>
          )}
        </div>
      )}
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">{Array.from({ length: 6 }).map((_, i) => <CardSkeleton key={i} />)}</div>
      ) : cars.length === 0 ? (
        <div className="animate-fade-in-up" style={{ animationDelay: '200ms' }}>
          <EmptyState icon={Car} message={t('cars.no_cars')} actionLabel={hasActiveFilters ? t('common.cancel') : undefined} onAction={hasActiveFilters ? clearFilters : undefined} />
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {cars.map((car, idx) => (<div key={car.id} className="animate-fade-in-up" style={{ animationDelay: `${(idx % 6) * 100}ms` }}><CarCard car={car} /></div>))}
          </div>
          {page < totalPages && (
            <div className="flex justify-center mt-12 md:mt-16">
              <Button onClick={() => { const next = page + 1; setPage(next); fetchCars(next, true) }} disabled={loadingMore} variant="outline" className="h-auto rounded-2xl border-slate-200 px-6 py-3 text-sm font-bold text-slate-900 shadow-sm hover:bg-slate-50 hover:shadow-md md:px-8 md:py-4 md:text-base">
