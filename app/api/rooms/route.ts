...
export async function POST(request: NextRequest) {
  try {
    ...
    const rawData = {
      ...
      cancellation_policy: (formData.get('cancellation_policy') as 'free' | 'partial' | 'non_refundable' | null) || undefined,
      cancellation_penalty_nights: formData.get('cancellation_penalty_nights') ? Number(formData.get('cancellation_penalty_nights')) : undefined,
      breakfast_included: formData.get('breakfast_included') === 'true',
      contact_phone: (formData.get('contact_phone') as string) || undefined,
      bedroom_count: formData.get('bedroom_count') ? Number(formData.get('bedroom_count')) : undefined,
      bathroom_count: formData.get('bathroom_count') ? Number(formData.get('bathroom_count')) : undefined,
      has_view: formData.get('has_view') === 'true',
    ...
    const { data: room, error: insertError } = await supabaseAdmin
      ...
      .insert({
        ...
        bedroom_count: parsed.data.bedroom_count ?? 1,
        bathroom_count: parsed.data.bathroom_count ?? 1,
    ...
    logActivity('room_created', { userId: user.id, metadata: { roomId: room.id } })
    ...
    after(async () => {
      try {
        await supabaseAdmin.from('provider_points_transactions').insert({
          ...
          description_ar: 'نقاط إضافة عرض سكن جديد',
          description_en: 'Points for adding a new room offer',
        })
        ...
        await notify({
          ...
          bodyAr: 'حصلت على 200 نقطة لإضافة عرض سكن جديد',
          bodyEn: 'You earned 200 points for adding a new room offer',
